module SneakersHelper
  def self.each_process_with_index(reverse = false, &block)
  ...
  def self.pid_file_exists?(pid_file)
    test(*("[ -f #{pid_file} ]").split(' '))
  end

  def self.process_exists?(pid_file)
    test(*("kill -0 $( cat #{pid_file} )").split(' '))
  end

  def self.quiet_sneakers(pid_file)
  ...
  def self.stop_sneakers(pid_file)
  ...
  def self.start_sneakers(pid_file, idx = 0)
    if fetch(:sneakers_run_config) == true
  ...
  def self.switch_user(role, &block)
